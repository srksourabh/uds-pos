/*
  ...
  ## Purpose
  These functions optimize the assignment algorithm by:
  - Pre-aggregating stock counts per bank
  - Counting active calls efficiently
  - Providing last assignment timestamp for tie-breaking
  - Supporting bulk assignment queries

  ## Security
  - SECURITY DEFINER to access all data
  - Called only by authenticated admin users
  - Used by assign-calls Edge Function
*/
...
BEGIN
  ...
  SELECT 
    ...
    -- Aggregate stock counts by bank as JSONB
    COALESCE(
      (
...
BEGIN
  ...
  -- Check stock if call requires device
